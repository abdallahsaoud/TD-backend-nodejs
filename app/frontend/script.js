...
async function loadUserFiles() {
    ...
    try {
        ...
        files.forEach(file => {
            ...
            fileElement.classList.add('list_link');
            fileElement.innerHTML = `
                <div class="upload-text">
                    <p><strong>${file.filename}</strong> (${(file.size / 1024).toFixed(2)} KB)</p>
                </div>
                <div class="button-container">
                    <button onclick="generateShareLink(${file.id})" class="upload-icon file-link"><img id="img-button" src="img/link.png" alt="Générer un lien"></button>
                    <button onclick="deleteFile(${file.id})" class="upload-icon file-link"><img id="img-button" src="img/trash.png" alt="Supprimer"></button>
                </div>
            `;
